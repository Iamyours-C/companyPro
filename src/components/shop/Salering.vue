<template>
  ...
  >
    ...
    <!-- 第三部分 -->
     <div class="operate-btns clearfix" v-if="saleringLists.length > 0">
       <div class="operate-goods-btn fl mar-r-40"><a @click="deleteAll">批量删除</a></div>
       <div class="operate-goods-btn fl"><a @click="isDownAll">批量下架</a></div>
     </div>
     <!-- 第四部分 -->
     <div v-if="saleringLists.length > 0">
       <ul class="order-describ-box text-center text-color-6 clearfix">
        <li class="fl describ-name check-all-box">
          ...
          <div class="check-all-order clearfix">
            <input type="checkbox" v-model="checkAll" class="fl" @click="checkedAll">
     ...
     <!-- 出售中的宝贝 -->
     <div class="salering-goods-box" v-if="saleringLists.length > 0">
       <ul v-for="(item, index) in saleringLists" :key="index">
          <li>
            <div>
              <!-- 订单抬头 -->
              <div class="order-head salering-head">
                <input type="checkbox" v-model="item.status" @click="singleCheck(item)">
              </div>
              <!-- 订单商品 -->
              <div class="order-body text-color-6">
                ...
                <div class="order-body-text">
                  ...
                  <div class="mar-t-40 salering-operate-text">
                    <p class="pad-tb-4" v-if="item.isUp === '1'"><a @click="isDownGoods">下架</a></p>
     ...
     <!-- 无数据时显示 -->
     <div class="show-icon" v-if="saleringLists.length == 0">
       <div class="show-box">
         <div class="icon-box">
           <img src="../../img/order.png" />
         </div>
         <div class="warning-notice text-center">暂无出售中商品！</div>
       </div>
     </div>
     <!-- 分页按钮 -->
...
export default {
  ...
  methods: {
      ...
      getGoodsStore({
        ...
        success: (res) => {
          if (res.code === 0) {
            ...
            _this.saleringLists = res.data.list
          }
        },
        error: (error) => {
          console.log(error)
    ...
    singleCheck (item) {
      ...
      if (_this.checknum === _this.saleringLists.length) {
        _this.checkAll = true
      } else {
