    ...
    <div class="content">
      <div class="certify-status">
        ...
        <div class="certify-status-notice">
          <div class="certify-status-title font-24 text-color-6">货车信息</div>
        </div>
        <!-- 状态标志 -->
...
<script>
import {truckInfoCertify} from '../../common/sendAxios.js'
export default {
  data () {
    return {
      message: '',
  ...
  methods: {
    ...
    // 返回提交货车信息
    returnSubmitInfo () {
      this.$router.push({name: 'editTruckInfo', query: {data: this.data}})
    },
    // 返回我的货车
    returnTruckInfo () {
